        ...
        {#if tableData.length}
            ...
            {#each tableData as row, index}
                ...
                {#each columnOrder as field, colIdx}
                    ...
                    <div class="col field" on:contextmenu|preventDefault={(e)=> handleRowContextMenu(e, row, field)} class:row-highlighted={mouse.row === index+1} class:col-highlighted={mouse.col === colIdx+1} class:even-row={index % 2 === 0} class:clickable={config.columns[field].onclick !== undefined} on:click={config.columns[field].onclick ? (e)=> config.columns[field].onclick(row[field], row._originalData, e) : ()=> true} on:keypress on:mouseenter={()=>setColRow(colIdx+1,index+1)} on:mouseleave={()=>setColRow(undefined, undefined)} on:focus>
